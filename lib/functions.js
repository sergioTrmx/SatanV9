...
const ss = async(query) => { // Um print de site
    request({
        ...
        encoding: "binary",
        qs: {
            access_key: "https://api.apiflash.com/v1/urltoimage?access_key=e03d8ce2eb6e4fda8ee7b7b196ee1adb&format=jpeg&fresh=true&ttl=1&url=http%3A%2F%2Fwww.petardas.com%2F", // Insira uma api do site apiflash url to image dentro das aspas
            url: query
        }
    }, (error, response, body) => {
        if (error) {
